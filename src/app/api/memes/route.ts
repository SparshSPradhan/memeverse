...
// // Define the Meme interface for better type safety
// interface Meme {
//   id: string;
//   image: string;
//   caption: string;
...
//     // Map the response to match your format
//     const memes: Meme[] = data.data.memes.map((meme: { id: string; url: string; name: string }) => ({
//       id: meme.id,
//       image: meme.url,
//       caption: meme.name,
//       likes: Math.floor(Math.random() * 100),
...
//       totalPages: Math.ceil(memes.length / limit),
//     });
//   } catch (_error) {  // ✅ Suppressed unused variable warning
//     return NextResponse.json({ error: "Failed to fetch memes" }, { status: 500 });
//   }
// }
